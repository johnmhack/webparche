-- Solo búsqueda por código Parche (no por placa).
-- Evita que alguien mire una placa en la calle y consulte datos del motero.
...
AS $$
  SELECT COALESCE(
    (
      ...
      WHERE m.activa = true
        AND upper(trim(COALESCE(m.codigo_parche, ''))) = upper(trim(p_query))
      LIMIT 1
    ),
    'null'::json
  );
...
-- Compat: la vieja RPC por placa ahora también exige código (misma función)
